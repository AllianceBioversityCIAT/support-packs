...
export class SPTermsconditionsComponent implements OnInit, OnChanges {
  ...
  onSetEmail() {
    if (!this.emailForm.invalid && !this.step5) {
      this.spinner.show();
      this.tcService.getPersonInfo({ email: this.emailForm.value['email'] }).subscribe(
        (res) => {
          // console.log(res);
  ...
  onSetTC() {
    ...
    if (!this.tc_Form.invalid) {
      this.spinner.show();
      this.step4 = false;
      const params = this.tc_Form.value;
      ...
      console.log(params);
      this.tcService.setDownload(params).subscribe(
        (res) => {
          this.spinner.hide();
   ...
   */
  getRegions() {
    this.tcService.getRegions().subscribe(
      (res) => {
        this.regions = res;
        // console.log(res)
      },
