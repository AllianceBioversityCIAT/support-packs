   ...
   async getGuidelineByIdWithoutImportantLevels(app_id: string): Promise<any> {
    try {
        ...
        guidelines.forEach(guideline => {
            guideline.resources = resourcesGuidelines.filter(resource => resource.guideline_id === guideline.id);
        });

        return guidelines;
    } catch (error) {
        return { error: 'An error occurred while fetching guidelines without important levels.', details: error };
    }
}
    ...
    try {
        ...
        if (guidelines.length > 0) {
            ...
            guidelines.forEach((guideline: any) => {
                guideline.resources = resourcesGuidelines.filter((resource: any) => resource.guideline_id === guideline.id);
            });
        } else {
        ...
        return { error: 'An error occurred while fetching guidelines.', details: error };
    }
}
